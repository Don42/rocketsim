...
def main():
  ...
  dt = 1.0/sr
  simTimeLimit  = 200      # Timelimit [s]
  rocket = Rocket(dt)
  t = [0]       #time in seconds
  burnTime = -1 * (rocket.mFuel[0] / rocket.dm)
  lastBurn = 0
  #while dt * rocket.index < 200 + burnTime:
  while rocket.height[rocket.index] > 0.0 and rocket.index * dt < 1000:
    t.append(t[rocket.index]+dt)
    rocket.step()
    if(rocket.height[rocket.index] <= 120 and not rocket.firing):
      rocket.firing = True
    if(rocket.height[rocket.index] <= 1000 and not rocket.parachute):
      rocket.parachute = True
    if(rocket.height[rocket.index] <= 1):
      rocket.dm = -7
...
class Rocket():
  mRocket     = 900   #131000.0  # mass of the rocket without fuel [kg]
  mFuel       = [230] #[2169000.0] # inital mass of the fuel [kg]
  dm          = -6    #-13360.0  # flow of mass from the rocket [kg/s]
  Cd          = 0.5       # Drag Coefficient
  A_front     = 3.0       # frontal surface area [m**2]
  Cd_para     = 0.5
  A_para      = 8.0
  specImpulse = 150  #263.0     # specific Impulse by weight [s] look at wikipedia
  height      = [20000.0]
  speed       = [-800.0]
  acceleration= [0.0]
  f__all      = [0.0]
  ...
  def calc_fuel(self, index):
    if self.firing:
      newFuel = self.mFuel[index-1] + self.dm * self.dt
      if newFuel <= 0:
        self.firing = False
        newFuel = 0
      return newFuel
  ...
  def f_thrust(self, index):
    if self.firing == True:
      return self.specImpulse * -1 * self.env.gravity(self.height[index]) * self.dm
    else:
      return 0
